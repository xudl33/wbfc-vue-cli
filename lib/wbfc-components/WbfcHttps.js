...
import qs from 'qs'   //qs 转化参数在后台表单格式提交时需要使用
import {getCookie,setCookie} from '../../static/js/cookie.js'  //引入cookie
...
var defaultsOptions = {
    ...
    config: {
      ...
      withJwt: true, // 允许使用cookie中的JWT Token
      timeout: 10000, // 超时时间
...
function formatReq(type, url, data, options) {
  ...
  if(opts.ignoreRepeat){
    ...
    var repeatToken = hex_md5(opts.url + opts.data);
        ...
        switch (err.response.status) {
          ...
          case 401:
            ...
            if (isErrInfo >= 0 || isErrInfo1 >= 0) {
              ...
              router.app.$router.push(defaultsOptions.routes['login']);
              err.message = null;
            } else {
